...
2. Errors can rapidly accumulate and snowball, as an incorrectly generated token becomes part of the context that all subsequent tokens rely on.
...
5. Hallucinations may get amplified with ARMs, because once an incorrect token is generated, it is used as context for subsequent generations, thereby leading the model to generate coherent narratives over incorrect facts.
...
Diffusion models are still generative models. Unlike masked language modeling where the masking rate is fixed during training, diffusion models randomly sample a masking rate (or “time”) between 0 and 1 for each example <d-cite key="nie2025largelldm"></d-cite>. 
...
1. Pre-train from scratch or perform continued pre-training
...
These problems can be resolved if the model can dynamically adapt its output length for each query. To this end, Li et al. <d-cite key="li2025beyondfixed"></d-cite> introduce the creatively named DAEDAL (**D**ynamic **A**daptive Length **E**xpansion for **D**iffusion L**a**rge **L**anguage Models), a training-free decoding technique that leverages `<EOS>` token probabilities to dynamically adjust output length. 
...
2. Linear - the weight increases with the number of steps, i.e. the later steps are weighted more
3. Exponential - similar to linear, except that the weight increases exponentially towards the end of the sequence
...
3. Volatility bands - these are bands of positions where the confidence is fluctuating. 
...
4. If the delimiter with the highest confidence surpasses a threshold $$T$$, it is treated as the end of the semantic step, and the block size is adapted such that the block ends at that delimiter
...
, where conditioning on nearby tokens provides more predictive power. An any-order model thus risks spending model capacity on modeling hard and unnatural orders. 
